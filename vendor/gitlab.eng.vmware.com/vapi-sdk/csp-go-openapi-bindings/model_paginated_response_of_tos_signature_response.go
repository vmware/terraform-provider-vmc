/*
 * Identity APIs
 *
 * A List of all identity and account management related APIs. Note: In order to use Bearer token as authentication method, you must include Bearer as a prefix to your token in the Authorize section.   Accepted format is: Bearer <authorization_token>
 *
 * API version: 1.0
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package openapiclient
// PaginatedResponseOfTosSignatureResponse struct for PaginatedResponseOfTosSignatureResponse
type PaginatedResponseOfTosSignatureResponse struct {
	TotalResults string `json:"totalResults,omitempty"`
	PrevToken string `json:"prevToken,omitempty"`
	NextToken string `json:"nextToken,omitempty"`
	Results []TosSignatureResponse `json:"results,omitempty"`
}
